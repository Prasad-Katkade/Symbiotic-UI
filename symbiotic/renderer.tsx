...
import { StyleSheet, Pressable, View, TouchableOpacity, Text, useWindowDimensions } from 'react-native';
...
export const SymbioticRenderer = ({ symName, tree, isEditMode, onEditClick, resetOperations }: RendererProps) => {
  ...
  const renderNode = (nodeId: string, isTopLevel = false): React.ReactNode => {
    ...
    const ShieldOverlay = () => (
      <View 
        style={[StyleSheet.absoluteFill, { zIndex: 9997, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(239, 68, 68, 0.3)' }]}
    ...
    const finalClassName = `${className || ''} ${dynamicClasses} ${ghostStyle}`.trim();
      ...
      if (renderShield) {
        return (
          <View key={node.id} style={{ position: 'relative' }}>
             <Component className={finalClassName} {...otherProps} {...reattachedFunctions} />
             <ShieldOverlay />
          </View>
        );
      }
